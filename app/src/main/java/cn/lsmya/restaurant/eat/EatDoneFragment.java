...
public class EatDoneFragment extends BaseFragment implements BaseRecyclerViewAdapter.OnChildViewClickListener, ApiClientResponseWithPullHandler.GetPullLayout {
    ...
    private static class ApiHandler extends ApiClientResponseWithPullHandler<EatDoneFragment> {
        public ApiHandler(EatDoneFragment the) {
            super(the);
        }
        ...
        @Override
        public boolean onSubmitBefore(EatDoneFragment the, ApiClientRequest request) {
            if (request.isRoute(ROUTE.ORDER_LIST)) {
