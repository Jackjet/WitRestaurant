...
public class PullRecyclerView extends RecyclerView {
    ...
    private PullRecyclerViewAdapter lfAdapter;
    private boolean isLoadMore;
    private boolean isRefresh = true;
    private OnItemClickListener itemListener;
    private float mLastY;//上一次Y 值
    private PullRecyclerViewFooter recyclerViewFooter;
    private PullRecyclerViewHeader recyclerViewHeader;
    ...
    public void setAdapter(Adapter adapter) {
        ...
        lfAdapter = new PullRecyclerViewAdapter(getContext(), adapter);
    ...
    private void updateHeaderHeight(float delta) {
        ...
        if (isRefresh && !mPullRefreshing) { // 未处于刷新状态，更新箭头
            if (recyclerViewHeader.getVisiableHeight() > mHeaderViewHeight) {
                recyclerViewHeader.setState(PullRecyclerViewHeader.STATE_READY);
            } else {
                recyclerViewHeader.setState(PullRecyclerViewHeader.STATE_NORMAL);
            }
        }
    }
    ...
    public void stopRefresh(boolean isSuccess) {
        if (mPullRefreshing) {
            if (isSuccess) {
                recyclerViewHeader.setState(PullRecyclerViewHeader.STATE_SUCCESS);
            } else {
                recyclerViewHeader.setState(PullRecyclerViewHeader.STATE_FRESH_FAILT);
            }
    ...
    private void updateFooterHeight(float delta) {
        ...
        if (isLoadMore) {
            if (height > PULL_LOAD_MORE_DELTA) {
                recyclerViewFooter.setState(PullRecyclerViewFooter.STATE_READY);
                mPullLoading = true;
            } else {
                recyclerViewFooter.setState(PullRecyclerViewFooter.STATE_NORMAL);
                mPullLoading = false;
                mPullLoad = false;
            }
        }
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (!mPullRefreshing && layoutManager.findFirstVisibleItemPosition() == 0) {
                    ...
                    if (isRefresh
                        ...
                        recyclerViewHeader.setState(PullRecyclerViewHeader.STATE_REFRESHING);
                ...
                if (isLoadMore && mPullLoading && layoutManager.findLastVisibleItemPosition() == lfAdapter.getItemCount() - 1
                    ...
                    recyclerViewFooter.setState(PullRecyclerViewFooter.STATE_LOADING);
                    mPullLoad = true;
                    startLoadMore();
                }
                resetHeaderHeight();
...
//        lfAdapter.notifyDataSetChanged();
    ...
    private void initWithContext(Context context) {
        ...
        recyclerViewHeader = new PullRecyclerViewHeader(context);
        recyclerViewFooter = new PullRecyclerViewFooter(context);
